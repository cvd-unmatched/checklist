...
(function(){
  ...
  async function loadItems(listId){ log('Data', 'Loading items for list ' + listId); try{ var r = await fetch('/api/lists/'+listId+'/items',{ headers:{ Authorization:'Bearer '+token }}); if(await handleAuthError(r)) return; if(!r.ok){ var error = await r.json().catch(function(){return{error:'Failed to load items'}}); log('Error', 'Failed to load items: ' + (error.error || 'Server error')); return; } var items = await r.json(); currentItems = items; var toShow = hideCheckedItems ? items.filter(function(i){ return !i.checked; }) : items; renderItems(toShow); updateHideCheckedButton(); log('Data', 'Loaded ' + items.length + ' items'); } catch(e){ log('Error', 'Failed to load items'); if (window.loggingEnabled) console.error('Failed to load items'); } }
  function updateHideCheckedButton(){ var btn = document.getElementById('hideCheckedBtn'); if(!btn) return; btn.textContent = hideCheckedItems ? '\u2705 Show all' : '\ud83d\udce6 Hide packed'; btn.title = hideCheckedItems ? 'Show all items including packed' : 'Hide items you have already packed'; }
  function toggleHideChecked(){ hideCheckedItems = !hideCheckedItems; var toShow = hideCheckedItems ? currentItems.filter(function(i){ return !i.checked; }) : currentItems; renderItems(toShow); updateHideCheckedButton(); log('UI', hideCheckedItems ? 'Hiding packed items' : 'Showing all items'); }
  function renderItems(items){ var container = document.getElementById('itemsContainer'); if(!container) return; if(!items.length){ container.innerHTML = hideCheckedItems ? '<div class="muted">All packed! \u2705 Uncheck items or click \u201cShow all\u201d to see everything.</div>' : '<div class="muted">No items yet.</div>'; return; } container.innerHTML = items.map(function(item){ return '<div class="item big-checkbox" draggable="true" data-item-id="' + item.id + '" ondragstart="handleDragStart(event)" ondragover="handleDragOver(event)" ondrop="handleDrop(event)" ondragend="handleDragEnd(event)">' + '<span class="drag-handle" style="cursor: grab; user-select: none; margin-right: 8px; font-size: 18px; color: var(--muted);" title="Drag to reorder">☰</span>' + '<input type="checkbox" ' + (item.checked?'checked':'') + ' onchange="toggleItem(' + item.id + ', this.checked)" ondragstart="event.stopPropagation(); return false;">' + '<div class="item-label' + (item.checked?' checked':'') + '" data-item-id="' + item.id + '" style="'+(item.checked?'text-decoration:line-through; opacity:.85':'')+'" ondblclick="startEditLabel(' + item.id + ')" title="Double-click to edit">' + item.label + '</div>' + '<input type="number" class="qty-input row-control" value="' + item.quantity + '" min="1" ondragstart="event.stopPropagation(); return false;">' + '<button class="row-control btn-primary" onclick="updateQuantity(' + item.id + ', this.previousElementSibling.value)" ondragstart="event.stopPropagation(); return false;">Update</button>' + '<button class="row-control btn-accent2" onclick="startEditLabel(' + item.id + ')" ondragstart="event.stopPropagation(); return false;">\u270f\ufe0f Edit</button>' + '<button class="row-control btn-danger" onclick="deleteItem(' + item.id + ')" ondragstart="event.stopPropagation(); return false;">Delete</button>' + '</div>'; }).join(''); }
  ...
  function startEditLabel(itemId){
    ...
    itemRow.setAttribute('draggable', 'false');

    // Hiding the qty/Update/Edit/Delete controls (instead of fighting them for
    // flex space) guarantees the text field gets the full row width, even on
    // mobile browsers where a flex-basis:100% trick was still being squeezed.
    var hiddenControls = Array.prototype.slice.call(itemRow.querySelectorAll('.row-control'));
    hiddenControls.forEach(function(el){ el.classList.add('hidden'); });
    ...
    saveBtn.type = 'button';
    saveBtn.className = 'row-control btn-primary label-edit-action';
    saveBtn.textContent = '✅ Save';
    var cancelBtn = document.createElement('button');
    cancelBtn.type = 'button';
    cancelBtn.className = 'row-control label-edit-action';
    cancelBtn.textContent = 'Cancel';

    function cleanup(){
      input.remove(); saveBtn.remove(); cancelBtn.remove();
      hiddenControls.forEach(function(el){ el.classList.remove('hidden'); });
      itemRow.setAttribute('draggable', 'true');
    }
    function cancelEdit(){ labelEl.textContent = currentLabel; labelEl.style.display = ''; cleanup(); }
    function commitEdit(){ saveLabelEdit(itemId, input.value, labelEl, cleanup); }
    ...
    labelEl.insertAdjacentElement('afterend', cancelBtn);
    labelEl.insertAdjacentElement('afterend', saveBtn);
    labelEl.insertAdjacentElement('afterend', input);
    input.focus();
    input.select();
  }
  async function saveLabelEdit(itemId, newLabel, labelEl, cleanup){ if(!newLabel || newLabel.trim() === ''){ newLabel = currentItems.find(function(x){ return x.id === itemId; }).label; } newLabel = newLabel.trim(); log('Edit', 'Updating label for item ' + itemId + ' to: ' + newLabel); try{ var r = await fetch('/api/items/'+itemId,{ method:'PUT', headers:{'Content-Type':'application/json', Authorization:'Bearer '+token}, body: JSON.stringify({ label: newLabel })}); if(await handleAuthError(r)) return; if(r.ok){ labelEl.textContent = newLabel; labelEl.style.display = ''; cleanup(); log('Edit', 'Label updated successfully'); var item = currentItems.find(function(x){ return x.id === itemId; }); if(item){ item.label = newLabel; } } else { var error = await r.json().catch(function(){return{error:'Update failed'}}); log('Error', 'Failed to update label: ' + (error.error || 'Server error')); alert('Failed to update label: ' + (error.error || 'Server error')); cleanup(); await loadItems(currentListId); } } catch(e){ log('Error', 'Failed to update label'); if (window.loggingEnabled) console.error('Failed to update label', e); alert('Failed to update label'); cleanup(); await loadItems(currentListId); } }
