...
(function(){
  ...
  async function createList(){ var name = document.getElementById('newListName').value; var startDate = document.getElementById('newStart').value; var endDate = document.getElementById('newEnd').value; if(!name) return; if (startDate && endDate && endDate < startDate) { if (window.loggingEnabled) console.warn('Invalid date range'); alert('End date cannot be before start date'); return; } log('Create', 'Creating list: ' + name + ' (' + startDate + ' to ' + endDate + ')'); try{ var r=await fetch('/api/lists',{ method:'POST', headers:{'Content-Type':'application/json', Authorization:'Bearer '+token}, body: JSON.stringify({ name:name, startDate:startDate || null, endDate:endDate || null })}); if(r.ok){ document.getElementById('newListName').value=''; document.getElementById('newStart').value=''; document.getElementById('newEnd').value=''; log('Create', 'List created successfully'); loadLists(); } else { var err=await r.json().catch(function(){return{error:'Create failed'}}); alert(err.error||'Create failed'); } }catch(e){ log('Error', 'Failed to create list'); if (window.loggingEnabled) console.error('Failed to create list'); } }
  ...
  async function saveEdits(){ if(!currentListId) return; var name=document.getElementById('editName').value; var start=document.getElementById('editStart').value; var end=document.getElementById('editEnd').value; if(!name) return; if (start && end && end < start) { if (window.loggingEnabled) console.warn('Invalid date range'); alert('End date cannot be before start date'); return; } log('Edit', 'Saving edits: ' + name + ' (' + start + ' to ' + end + ')'); try{ var r=await fetch('/api/lists/'+currentListId,{ method:'PUT', headers:{'Content-Type':'application/json', Authorization:'Bearer '+token}, body: JSON.stringify({ name:name, startDate:start||null, endDate:end||null })}); if(r.ok){ var updated = await r.json(); lists = lists.map(function(l){ return l.id===currentListId ? { id:l.id, name:updated.name, start_date:updated.startDate, end_date:updated.endDate } : l }); log('Edit', 'Edits saved successfully'); renderListsGrid(); openList(currentListId); toggleEdit(); } else { var err = await r.json().catch(function(){return{error:'Update failed'}}); alert(err.error||'Update failed'); } }catch(e){ log('Error', 'Failed to save edits'); if (window.loggingEnabled) console.error('Failed to save edits'); } }
